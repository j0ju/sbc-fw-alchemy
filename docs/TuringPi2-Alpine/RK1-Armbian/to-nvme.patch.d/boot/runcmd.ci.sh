...
#- config locales
  sed -n -r -e "/(en_GB|en_US|de_DE|en_DK)\.UTF-8/ s/#? *// p" \
    < /etc/locale.gen > /etc/locale.gen.ci
  #if ! cmp /etc/locale.gen /etc/locale.gen.ci > /dev/null; then
    mv -f /etc/locale.gen.ci /etc/locale.gen
    locale-gen
  #fi
  rm -f /etc/locale.gen.ci
...
#- pre-seed authorized_keys if avail
if [ -f ~/.ssh/authorized_keys ] && homes=$(ls -d /home/*); then
  for home in $homes; do
    mkdir -p "$home/.ssh"
    user="${home##*/}"
    cp ~/.ssh/authorized_keys "$home/.ssh"/authorized_keys
    chown "$user:" "$home/.ssh"
    chmod 744 "$home/.ssh"
