...
PKGS=" openssh-server openssh-server-common openssh-server-common-openrc openssh-keygen "
...
# update rc-files
# chroot /target rc-update add sshd default 1> /dev/null || :
# chroot /target /usr/local/sbin/update-rc 1> /dev/null || :

! chroot /target which etckeeper 2> /dev/null 1>&2 || \
